...
# Audio processors and inference engines per session
...
async def startup_event():
    ...
    global config, models, session_manager, device
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        if vis_models:
            ...
            # Convert motion_anchor to tensor (expected shape: (1, 512) or (512,))
            anchor_tensor = torch.from_numpy(session.motion_anchor).float().to(device)

            # Load reference image (session-specific)
        ...
        while True:
            ...
            if message_type == 'text':
                ...
                # Handle binary messages (audio chunks, images)
                await handle_binary_message(session_id, data)
...
async def handle_binary_message(session_id: str, data: bytes):
    """Handle binary messages (audio chunks, images)."""
    try:
        ...
        if msg_type == 0x01:
            # Audio chunk
            await handle_audio_chunk(session_id, data[1:])

        elif msg_type == 0x02:
            # Reference image
            await handle_reference_image(session_id, data[1:])
...
async def handle_audio_chunk(session_id: str, audio_data: bytes):
    """
    Queue audio chunk for processing by generation worker.
    This is now a lightweight operation - just queue the audio.
    """
    session = session_manager.get_session(session_id)
    ...
    try:
        ...
        # Queue audio for worker processing
        if session_id in generation_workers:
...
async def frame_sender_loop(session_id: str):
    """
    Continuously send generated frames to the client.
    Runs in parallel with audio reception.
    """
    logger.info(f"Frame sender loop started for session {session_id}")

    try:
        while session_id in generation_workers:
            # Get next frame from worker (non-blocking with timeout)
            frame_bytes = await generation_workers[session_id].get_next_frame(timeout=0.1)

            if frame_bytes is not None:
                # Send frame to client
                # Frame message already encoded in worker
                await connection_manager.send_bytes(session_id, frame_bytes)
...
async def handle_reference_image(session_id: str, image_data: bytes):
    ...
    try:
        ...
        # Store pending reference image for next session (no immediate update)
